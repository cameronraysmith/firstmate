...
#
# A forge-side merge is confirmed actually merged before it is reported; an
# auto-merge-queued or unconfirmed request leaves the poll armed and records no
# landed outcome. A local fast-forward carries its own proof instead, re-reading
# the base branch to show it contains the validated head.
...
case "$PROVIDER" in
  github)
    if [ "$LANDING" = forge ]; then
      ...
      # its landed state is confirmed before any outcome is reported. A local
      # fast-forward needs no such confirmation: land_local_fast_forward pushes
      # the head onto the base and re-reads the base to prove it contains that
      # head, so the landing is already established when it returns.
      github_confirm_rc=0
