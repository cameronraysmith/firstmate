...
# (see crew_absorb_class in bin/fm-classify-lib.sh): a declared wait, either a
# paused: external wait or a verified captain-held transfer, re-surfaces only on
# its long bounded cadence, although its initial no-verb status signal still
# surfaces in normal mode; a crew that FINISHED and already said so in its own
# status log is absorbed outright, since its terminal line reached firstmate
# through the signal path and repeating it as a stale wake costs a supervision
# turn for every poll the captain's approval stays outstanding; and a read that
# found NO current-state source at all is timed toward the ordinary wedge
# threshold rather than reported as a stopped crew, because an absent source is
# the absence of evidence and not evidence of a wedge.
...
#                          wait, either an external-wait pause or a verified
#                          captain-held transfer, is absorbed instead with its own
#                          long re-surface cadence, never as a wedge, and that
#                          recheck reason names which human the wait is on; a live
#                          pane behind that declaration surfaces once, then takes
#                          the cadence. A crew whose reconciled state is FINISHED
#                          and whose own status log already reports that finish is
#                          absorbed with no wedge timer. Only when no absorb class
...
# matches reality, on the one path where liveness can still be read; a secondmate
# earns the cadence on its declaration alone, because its endpoint liveness is
...
pause_state_class() {  # <window> <task>
  ...
  # `unknown` joins `none` here because a dead endpoint is one of the ways the
  # reconciler ends up with no source to read at all; `terminal` does not, because
  # a crew that finished needs no recheck cadence.
  case "$class" in
    none|unknown) class=paused ;;
  esac
  case "$class" in
    ...
    if [ "$h" = "$prev" ]; then
      ...
      if [ "$n" -ge 2 ] && [ "$busy_now" -ne 0 ]; then
        ...
        if [ "$kind" = secondmate ]; then
          ...
          #     declared wait pause_state_class does not admit. Surface once so
