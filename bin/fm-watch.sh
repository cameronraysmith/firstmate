...
#                          re-surface cadence, never as a wedge; a live pane
#                          behind that declaration surfaces once, then takes the
#                          cadence. A crew whose reconciled state is FINISHED and
...
# has confidently exited, while a live or ambiguously read agent still surfaces
# once before joining it - one supervision turn of insurance that the declaration
# matches reality, on the one path where liveness can still be read.
...
# cadence - from the idle path once a determination has already surfaced, and
# from busy_turn_bound_check on a live busy pane - so it must be
    ...
    if [ "$h" = "$prev" ]; then
      ...
      if [ "$n" -ge 2 ] && [ "$busy_now" -ne 0 ]; then
        ...
        if [ "$kind" = secondmate ]; then
          ...
          #     has until then to come back - and, once it declares that wait,
          #     joins the bounded pause cadence instead of the wedge timer.
          #   - none: no running pipeline, no exact busy verdict, and a source
          #     that DID answer with something other than work in progress - or a
          #     declared pause whose agent is NOT confidently dead. Surface once so
          #     firstmate inspects the inconclusive state (it may be done via an
          #     interactive menu that wrote no done: status, waiting on a decision,
          #     or wedged) instead of leaving the finish to wait out the timer; a
          #     declared pause then takes the bounded cadence from the next
          #     determination on (see surface_nonterminal_stale).
          if [ "$(cat "$sf" 2>/dev/null || true)" != "$h" ]; then
            task=$(window_to_task "$w" "$STATE")
            case "$(pause_state_class "$w" "$task")" in
              working)
