...
# since the previous poll. Every other no-verb wake surfaces, so a crew that
# finishes (or stops and waits) is never silently swallowed. The stale path
# applies that same rule and three narrower absorb classes on top of it, each
# keyed to what bin/fm-crew-state.sh - the owner of current-state truth -
# actually reports (see crew_absorb_class in bin/fm-classify-lib.sh): a declared
# wait, either a paused: external wait or a verified captain-held transfer,
# re-surfaces only on its long bounded cadence, although its initial no-verb
# status signal still surfaces in normal mode; a crew that FINISHED and already
# said so in its own status log is absorbed outright, since its terminal line
# reached firstmate through the signal path and repeating it as a stale wake
# costs a supervision turn for every poll the captain's approval stays
# outstanding; and a read that found NO current-state source at all is timed
# toward the ordinary wedge threshold rather than reported as a stopped crew,
# because an absent source is the absence of evidence and not evidence of a wedge.
