...
#
# An empty result is never a reason to fall back to an ambient session and sweep
# a window this home may not own. The caller reaches this function only after
# finding presentation journals, so the warning names those task ids and the
# manual inspection needed to resolve records that name no usable session.
fm_herdr_cleanup_sessions() {
  local journal id meta session target sessions affected_ids
  sessions=$(
  ...
  if [ -z "$sessions" ]; then
    affected_ids=$(
      for journal in "$STATE"/*"$FM_BACKEND_HERDR_PRESENTATION_JOURNAL_SUFFIX"; do
        [ -f "$journal" ] && [ ! -L "$journal" ] || continue
        basename "$journal" "$FM_BACKEND_HERDR_PRESENTATION_JOURNAL_SUFFIX"
      done | sort -u | awk 'NF { if (ids != "") ids = ids ", "; ids = ids $0 } END { print ids }'
    )
    fm_herdr_cleanup_warn \
      "task IDs $affected_ids have projection journals but no recorded Herdr session; inspect those tasks in Herdr and retire only journals whose panes are confirmed gone"
    return 0
  fi
  printf '%s\n' "$sessions"
}
