...
cleanup_firstmate_home_children() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    rm -f "$sub_state/$child_id.turn-ended" \
      ...
      "$sub_state/$child_id.omp-ext.ts" "$sub_state/$child_id.atomic-ext.ts" \
      "$sub_state/$child_id.atomic-session" \
...
rm -f "$STATE/$ID.turn-ended" "$STATE/$ID.meta" \
  "$STATE/$ID.pi-ext.ts" "$STATE/$ID.omp-ext.ts" "$STATE/$ID.atomic-ext.ts" \
  "$STATE/$ID.atomic-session" "$STATE/$ID.grok-turnend-token" \
