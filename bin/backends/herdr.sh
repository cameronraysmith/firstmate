...
fm_backend_herdr_projection_close_pane_focus_preserving() {  # <session> <pane-id> [required-agent-state]
  ...
  if [ "$target_tab" = "$active_tab" ]; then
    echo "warning: herdr presentation cleanup target tab $target_tab is the focused tab of herdr session $session; refusing a close that cannot preserve that session's focus" >&2
    return 1
  fi
