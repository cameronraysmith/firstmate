...
family_for_basename() {
  case "$1" in
    ...
    fm-atomic-adapter.test.sh|fm-atomic-harness.test.sh|\
    ...
    fm-send-popup-settle.test.sh|fm-send-settle.test.sh|fm-stat-lib.test.sh|\
    ...
    fm-backend-herdr-session-per-project-e2e.test.sh|\
    ...
    fm-atomic-adapter-live-e2e.test.sh|\
    ...
    fm-omp-branch-capability.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    docs/configuration.md|docs/supervision-protocols/*)
      printf '%s\n' pure-contract-unit
      ;;
    tests/lib.sh|tests/*-helpers.sh|tests/cleanup-safety.sh)
