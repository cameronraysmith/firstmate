...
`fm-pr-merge.sh` lands a PR as a local fast-forward by default, keeping the branch head byte-identical to the commit CI validated and preserving the branch's own commits; a forge-side merge lands a commit CI never ran on, so it requires the captain's explicit in-the-moment authorization and is requested by passing an explicit merge method.
