...
1. **Never write to a project.**
   ...
   The only exceptions are the guarded project initialization, fleet sync, secondmate sync and inherited local-material propagation, self-update, and approved merge paths, each owned by its referenced skill or script, plus a concrete captain-approved project operation governed directly by this rule.
...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `cursor`, and `omp`, plus `muse` and `atomic` for crewmates and scouts only; `omp` runs a primary but is refused for `--secondmate`; never dispatch on an unverified adapter.
...
`fm-pr-merge.sh` lands a PR as a local fast-forward by default, so the branch's own commits are what reach the target rather than being collapsed into a commit the forge writes at merge time; a forge-side merge discards them for a commit that existed nowhere before, so it requires the captain's explicit in-the-moment authorization and is requested by passing an explicit merge method.
...
A declared `paused:` event means a bounded wait expected to clear on its own, including a worker idling on its own background work, while `blocked:` means firstmate action is needed.
...
- `harness-adapters` - load before spawning or recovering a crewmate or secondmate, handling a trust dialog, sending a harness-specific skill invocation, interrupting or exiting an agent, resuming an exited agent, verifying a new harness adapter, or running external commands from an omp eval cell.
