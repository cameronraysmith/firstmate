...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `cursor`, and `omp`, plus `muse` for crewmates and scouts only; `omp` runs a primary but is refused for `--secondmate`; never dispatch on an unverified adapter.
