...
test_non_cursor_launch_clears_inherited_cursor_markers() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "$(fm_launch_marker_prefix)" \
...
test_pi_threads_model_and_max_effort() {
  ...
  assert_not_contains "$launch" "FM_FIRSTMATE_PI_LAUNCH_BRIEF=" \
    "pi launch still exports the removed input-reroute binding"
...
test_claude_forwards_firstmate_config_dir_when_set() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "CLAUDE_CONFIG_DIR='/opt/test/claude-work' $(fm_launch_marker_prefix)CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false claude" \
