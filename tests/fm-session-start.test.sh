...
run_session_start() {
  ...
  if [ -n "$pi_harness" ]; then
    env -u CLAUDECODE -u OMPCODE -u ATOMIC_CODING_AGENT -u GROK_AGENT PI_CODING_AGENT=true FM_PI_HARNESS="$pi_harness" \
      FM_HOME="$home" FM_ROOT_OVERRIDE="$root" PATH="$path" \
      "$SESSION_START"
  else
    env -u CLAUDECODE -u OMPCODE -u ATOMIC_CODING_AGENT -u PI_CODING_AGENT -u FM_PI_HARNESS -u GROK_AGENT \
      FM_HOME="$home" FM_ROOT_OVERRIDE="$root" PATH="$path" \
      "$SESSION_START"
  fi
}
...
run_pi_session_start() {  # <home> <root> <path> [fm-session-start args...]
  ...
  shift 3
  env -u CLAUDECODE -u OMPCODE -u ATOMIC_CODING_AGENT -u GROK_AGENT PI_CODING_AGENT=true FM_PI_HARNESS=pi \
