...
test_stale_enqueue_before_suppressor() {
  ...
  if fm_stat_is_gnu; then sig=$(stat -c '%s:%Y' "$state/stale.status"); else sig=$(stat -f '%z:%Fm' "$state/stale.status"); fi
...
test_not_working_stale_enqueue_before_suppressor() {
  ...
  if fm_stat_is_gnu; then sig=$(stat -c '%s:%Y' "$state/stopped.status"); else sig=$(stat -f '%z:%Fm' "$state/stopped.status"); fi
