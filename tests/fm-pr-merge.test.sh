...
#   (af) a successful merge in a secondmate home reports the landed PR upward
#        once, on the route its parent binding names, and a repeat merge of the
#        same PR does not duplicate that line
#   (ag) a refused or failed merge reports nothing
#   (ah) a successful merge in a main home leaves a durable wake naming the PR
#   (ai) a secondmate home with no usable parent binding says so loudly instead
#        of merging in silence
#   (aj) an accepted queued GitHub merge emits nothing and leaves its poll armed
#   (ak) an accepted queued GitLab merge emits nothing and leaves its poll armed
#   (al) an uncommitted marker retry never loses the durable outcome
#   (am) distinct merged PRs for a reused task each survive queue deduplication
set -u
