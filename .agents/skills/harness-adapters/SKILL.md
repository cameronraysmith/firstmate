...
|---|---|---|---|
| claude | `--model <id>` (canonical prefix stripped) | `--effort <low\|medium\|high\|xhigh\|max>` | Verified on Claude Code 2.1.196; model form re-verified 2026-08-19 on 2.1.235: `--help` documents "an alias for the latest model (e.g. 'fable', 'opus', or 'sonnet') or a model's full name" with no provider prefix. |
| codex | `--model <id>` (canonical prefix stripped) | `-c 'model_reasoning_effort="<low\|medium\|high\|xhigh>"'` | Verified on codex-cli 0.142.1; model form re-verified 2026-08-19 on 0.148.0: `-m/--model <MODEL>` takes a bare id everywhere (its own config example is `-c model="o3"`) and exposes no provider prefix or `--provider` flag. The installed binary schema contains `model_reasoning_effort`, the active config uses it, and the bundled model catalog advertises only low/medium/high/xhigh. `max` is omitted. |
| grok | `--model <model>` (canonical translation unverified on the 2026-08-19 host; verbatim until then) | `--reasoning-effort <low\|medium\|high>` | Verified on grok 0.2.99 (2026-07-13). `--effort` is an alias, but firstmate's profile axis is reasoning effort. As of 0.2.99 the ceiling is `high`; both `xhigh` and `max` are rejected with `use one of: high, medium, low`, so firstmate omits them. |
| pi / pi-signed | `--model <provider/id>` (pair-native, verbatim) | `--thinking <low\|medium\|high\|xhigh\|max>` | Verified 2026-07-27 on Pi and pi-signed 0.82.0; both expose the same accepted thinking levels and completed the same model-qualified max-thinking smoke. Model form re-verified 2026-08-19 on pi 0.84.2: `--help` documents `--model <pattern>` "supports 'provider/id'". pi-signed launches the same pi engine and shares the verified surface. |
| omp | `--model <provider/id>` (pair-native, verbatim) | `--thinking <low\|medium\|high\|xhigh\|max>` | Verified 2026-08-17 on omp 17.3.5: `--help` documents the combined shape (`"openai/gpt-5.2"`) and calls its separate `--provider` flag legacy. It also documents `off\|minimal\|low\|medium\|high\|xhigh\|max\|auto`, so the whole shared vocabulary maps across. An unknown LEVEL is accepted silently and falls back to the model default rather than refusing, so never pass a value outside the allowlist; an unknown MODEL does refuse the launch. The applied level is additionally clamped to what the selected model advertises in `omp models`. |
| opencode | `--model <provider/model>` (pair-native, verbatim) | none for firstmate's interactive launch | Verified on opencode 1.17.6; model form re-verified 2026-08-19: `--help` documents "-m, --model model to use in the format of provider/model". `opencode run` has `--variant`, but firstmate launches the interactive `opencode --prompt` path, which has no verified effort flag. |
| kimi | `--model <model>` (canonical translation unverified on the 2026-08-19 host; verbatim until then) | none | Verified 2026-07-25 on Kimi Code CLI 0.29.1. |
| cursor | `--model <id>` (canonical prefix stripped) | none | Verified 2026-08-11 on Cursor Agent CLI 2026.08.11-e8db854; model form re-verified 2026-08-19 on that build: `--help` documents "Model to use (e.g., gpt-5, sonnet-4-thinking)", bare ids with no provider prefix. No effort flag exists, so firstmate records the requested effort in task metadata and omits it from the launch. `fm-spawn` preflights the TRANSLATED bare id against `cursor-agent --list-models` rather than assuming a low/medium/high family: the live catalog carries only `-high` Grok ids. |
| muse | `--model <model>` (canonical translation unverified on the 2026-08-19 host; verbatim until then) | `--reasoning-effort <low\|medium\|high\|xhigh>`, and `ultra` only for an explicit `max` | Verified 2026-08-05 on Muse Code 0.1.0-R708.1. The flag accepts `none\|minimal\|low\|medium\|high\|xhigh\|ultra` and defaults to `high`. `ultra` is muse's max-class level, so it is reachable only through an explicit captain `max`, never from the generic fallback; `none` and `minimal` sit below the shared vocabulary and stay unreachable. |
...
| omp | Run `omp models` (or `omp models find <substring>`, `omp models --json`), which lists every available provider/model and each one's supported thinking levels. `omp usage` reports the authenticated accounts. |
