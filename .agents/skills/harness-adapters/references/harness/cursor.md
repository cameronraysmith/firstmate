...
| Models | `--model <id>`, the canonical `provider/id` prefix stripped; model form re-verified 2026-08-11 on that build, whose `--help` documents "Model to use (e.g., gpt-5, sonnet-4-thinking)", bare ids with no provider prefix. `../../../bin/fm-spawn.sh` preflights the TRANSLATED bare id against current-account `cursor-agent --list-models` or legacy `agent --list-models`; the drifting observed list had only `cursor-grok-4.5-high` and `cursor-grok-4.5-high-fast` for Grok plus several `xhigh` ids, so choose a returned reasoning id and never assume low or medium Grok. |
