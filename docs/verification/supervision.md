...
Session-lock ownership in `bin/fm-session-lock-lib.sh` is decided by the session identity the lease records, because no pid names a session: Claude Code runs every background session of one home as a claimed spare under a single shared daemon, so the contiguous harness ancestry resolves the same outermost pid for all of them and pid equality reads as self-ownership for a session that never claimed the home.
The identity is read from `CLAUDE_CODE_SESSION_ID`, verified 2026-08-12 to reach an ordinary Bash tool subprocess under claude 2.x; `FM_SESSION_ID` overrides it, and a harness exposing neither falls back to the previous answer, ancestry membership, so the Stop auto-arm still reaches its lock owner wherever that owner sits.
Competing-holder liveness is narrowed the same way: a recorded process that is also an ancestor of the asking process is shared infrastructure, and one whose `fm_pid_identity` no longer matches has had its number reused, so neither keeps a departed session's home locked.
...
`tests/fm-session-lock-ancestry.test.sh` pins both platforms' reporting semantics behind a deterministic process table, runs the real Stop auto-arm in version-named, daemon-parented, and combined real process trees, and runs two real sessions under one shared daemon ancestor to prove exactly one of them acquires the lease, that the holder is never refused its own home, and that the surviving daemon does not lock out the next session.
