...
The unregisterable-harness fallback was added 2026-08-19 after a live reproduction: a working atomic crewmate (spinner running, `fm-crew-state.sh` reporting `working ... harness busy (atomic-ext)`) read `dead`, so interrupt refused it, because `herdr agent start --kind` accepts exactly pi, claude, codex, gemini, cursor, devin, agy, cline, omp, mastracode, opencode, copilot, kimi, kiro, droid, amp, grok, hermes, kilo, qodercli, and maki on the installed 0.8.0 client - atomic is not a kind, so the registry can never answer for its panes.
When the registry answers `agent_not_found` and the task's recorded harness is exactly that unlisted `atomic`, the classifier now accepts firstmate's own positive evidence (a generation-and-source-validated busy record from `atomic-ext`, or the pane's foreground process naming the atomic executable, whose live nix build reports `comm` as `node` with the rewrite in argv[0]) and reports `alive`; every registerable harness and every evidence-free or ambiguous read keeps its prior classification, so the recovery-licensing `dead` and `missing` verdicts are exactly as narrow as before.
The portable regressions pinning both directions are `tests/fm-secondmate-liveness.test.sh`'s `test_herdr_agent_state_unregisterable_fallback` and `test_herdr_agent_state_registerable_harness_unchanged`, which drive the real classifier over canned herdr responses and real busy-record files.
