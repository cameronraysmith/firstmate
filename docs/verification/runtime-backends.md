...
The structural fact refreshes with `herdr agent start --help` (the `[possible values: ...]` line under `--kind <KIND>`); run it after every Herdr upgrade and reconcile the list against `FM_BACKEND_HERDR_UNREGISTERABLE_HARNESSES` in `bin/backends/herdr.sh`.
