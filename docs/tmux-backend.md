...
An omp pane is the one place a cursor sitting ON a border row is still an input row, because omp carries its input inside the bottom border; that shape is admitted only when the foreground-process probe names omp, so the edge rule stands everywhere else.
