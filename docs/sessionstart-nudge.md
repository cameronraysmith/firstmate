...
| omp | Run | `.omp/extensions/fm-primary-turnend-guard.ts` injects with `pi.sendMessage` from `session_start` and `session_compact`. omp's `session_start` carries no `reason` field and fires once per omp PROCESS, so the source comes from the session header and argv instead: a `parentSession` header or a fork flag maps to `fork`, a continuation, resume-selection, or explicit-session flag alongside a header older than the current process maps to `resume`, and everything else is `startup`. | The custom message reaches model context without racing an initial positional prompt. `pi.sendMessage` throws at module scope, so injection only ever happens from inside a handler. Unlike Pi, this does NOT gate the first provider call: the digest is sent from inside `session_start`, which does not block the turn. Installed omp 18.0.10 does emit `before_agent_start`, so that gate is portable rather than absent - see the omp startup-context gating record in `docs/verification/runtime-backends.md`. |
| omp session replacement | Uncovered | None. | omp emits no lifecycle event for a same-process replacement: `/new` changes the session id while handlers keep firing. There is nothing to hook, so an omp primary does not re-emit its digest after a `/new`. Its compaction surface IS covered, unlike Cursor's. |
