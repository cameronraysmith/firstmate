...
| `fm-pr-merge.sh`         | Record PR metadata, then land a task's canonical full GitHub URL as a local fast-forward (or through the forge when a merge method is explicit), or merge a GitLab MR URL through a guarded `glab` path |
