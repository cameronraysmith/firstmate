...
| omp | `event.input.command` | `.omp/extensions/fm-primary-turnend-guard.ts` passes one `--command` argument and returns `{block: true}` only for exit 2. Same shape as Pi, measured on omp: a blocked bash call did not run and the model was shown the reason. |
