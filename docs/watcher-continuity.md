...
A numeric session-lock owner that fails the shared `fm_session_lock_holder_is_other_live_session` predicate is reclaimed through `bin/fm-lock.sh` before auto-arm state changes, while another live session's lock, an absent lock, or a malformed lock keeps the competing hook inert.
That predicate asks whether another SESSION is still live rather than whether the recorded pid is, because Claude Code's background sessions share one harness ancestor: reading that shared process's liveness as the holder's would leave a home unsupervised for as long as it ran.
...
`tests/fm-watcher-lock.test.sh` covers verified-successor attach, recovery publication before stale-lock removal, the typed self-eviction failure, bounded and successor-linked lifecycle rows, a SIGSTOP counterfactual that distinguishes a live PID from a stale beacon before classifying termination, and single-signal stand-down while the watcher still holds its own recovery locks.
