...
The push is never forced and a diverged PR branch is refused, so the only refs the path writes are a fast-forward of the base branch and the project clone's own remote-tracking refs.
