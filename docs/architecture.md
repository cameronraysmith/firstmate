...
`bin/fm-project-mode.sh` remains the one registry parser for the mechanical consumers that have no task in hand: fleet sync's `local-only` skip, home seeding's refusal and no-mistakes initialization, and the brief scaffold's `+planning` marker lookup.
...
Ship briefs prompt crewmates to create or update those files through the normal delivery path; `data/projects.md` stays a thin private registry, and a project registered with the `+planning` marker is the exception: its ship briefs carry a planning-context guard that forbids creating or modifying `AGENTS.md`/`CLAUDE.md` in the task worktree, because such projects keep them as machine-local symlinks into a planning repo and a fresh task clone contains neither.
