...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
Two stale reads are deliberately not actionable, because `bin/fm-crew-state.sh` already answers them and the stale path defers to that owner.
A crew whose reconciled state is a finish its own status log already reports is absorbed outright: that line reached the first mate through the captain-relevant signal path, the heartbeat backstop still owns it, and repeating it as a stale wake costs a supervision turn for every poll the captain's merge approval stays outstanding.
A finish that only the pipeline knows about is the opposite case and still surfaces, so a green pull request is never swallowed.
A read that found no current-state source at all starts the ordinary wedge timer instead of reporting a stopped crew, because a crew that ends its turn while its own background work runs blinds every source at once; an absent source is the absence of evidence, and a crew that really is frozen still escalates when the threshold passes.
...
A crew that declared a wait (`paused:`) or a verified captain-held transfer is the one exception to that bound: its busy verdict supplies liveness while identifying the long-running foreground call as the declared wait, so it takes the bounded `FM_PAUSE_RESURFACE_SECS` recheck instead of a wedge escalation.
...
The watcher and daemon share `bin/fm-classify-lib.sh` for captain-relevant status verbs, declared-wait vocabulary (a `paused:` wait and a verified `captain-held` transfer alike, through one combined predicate), and status-scan primitives.
...
Its default-on presentation projection may place one clean new task in a disposable workspace without changing endpoint authority or lifecycle ownership; [Presentation spaces](herdr-backend.md#presentation-spaces) owns that conditional design, the Herdr version floor its unconfigured default is gated behind, and its narrow home-local restored-shell cleanup at teardown and at locked session start.
...
PR-based task landings go through `bin/fm-pr-merge.sh`, which records `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh` before landing.
It requires a full canonical URL and rejects malformed URLs or repo override flags before recording merge state.
For a `https://github.com/<owner>/<repo>/pull/<n>` URL the default landing is a local fast-forward: it pushes the PR's own head commit onto the base branch, so every commit on the PR branch survives on the target instead of being collapsed, and the forge then marks the PR merged on its own.
The push is never forced and a diverged PR branch is refused, so the only refs the path writes are a fast-forward of the base branch, the project clone's own remote-tracking refs, and a private `refs/fm-merge/pull/<n>/head` ref pinning the PR head the forge actually serves.
An explicit `--squash`, `--merge`, `--rebase`, or `--method=<m>` selects a forge-side `gh-axi pr merge <n> --repo <owner>/<repo>` instead, which replaces the branch's commits with one the forge writes and therefore needs the captain's explicit authorization.
A `https://<host>/<path>/-/merge_requests/<n>` URL (see [docs/gitlab-merge-watch.md](gitlab-merge-watch.md)) invokes `glab mr merge <n> -R https://<host>/<path>`, so the instance comes from the URL, and adds no merge-method flag because the project's own merge method applies; a merge request never takes the local fast-forward, so `--local-ff` is refused there.
...
A per-project merge policy is not modeled: every project lands through those same shapes, and a repository whose own rules forbid a direct push to the base branch (branch protection, or a merge queue) needs the forge-side path chosen for it.
