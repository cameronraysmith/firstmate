...
Its default landing is a local fast-forward: it pushes the PR's own head commit onto the base branch, so that branch head stays byte-identical to the commit CI validated and every commit on the PR branch survives, and the forge then marks the PR merged on its own.
...
An explicit `--squash`, `--merge`, `--rebase`, or `--method=<m>` selects a forge-side `gh-axi pr merge <n> --repo <owner>/<repo>` instead, which lands a commit CI never ran on and therefore needs the captain's explicit authorization.
