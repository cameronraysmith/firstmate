...
Under the extension model - Pi, pi-signed, and omp - a live identity-matched watcher is the ordinary healthy state, but a genuinely unheld lock with a beacon fresh within grace is also healthy while a live session of that harness provably owns continuity, because the watch extension tears the watcher down on every actionable wake and spawns the replacement itself.
...
That ownership proof is `fm_primary_extension_owns_supervision` in `bin/fm-wake-lib.sh`: both of that harness's primary extensions must be recorded in their state markers at their current on-disk builds by the process named in `state/.lock`, and that process must still be alive.
The pair is keyed per harness by `fm_primary_extension_pairs`, and only the detected harness's own pair is accepted, because Pi's extensions load on omp and never fire there - Pi markers must never answer for an omp primary.
...
- omp BLOCKS instead. `.omp/extensions/fm-primary-turnend-guard.ts` listens for omp's Claude-shaped `session_stop`, forwards that payload's own `stop_hook_active` to the guard, and returns `{decision: "block", reason}` on exit 2, so a blind turn end is refused rather than followed up. omp awaits the async handler, and marks only the stop that FOLLOWS a block, so the default-mode loop guard above bounds it to one forced continuation per turn; `--claude` mode is deliberately not used. omp does not render the reason as pane text - it reaches model context, and the model's reaction is the visible signal.
